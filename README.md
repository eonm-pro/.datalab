...
Ce projet propose un environnement reproductible et isolé pour la manipulation de données en ligne de commande, basé sur Nix. Le lockfile garantit que les outils (et leurs versions exactes) seront identiques sur toutes les machines, sans nécessiter de conteneur, tout en restant compatible avec des environnements déjà conteneurisés comme Onyxia (SSP Cloud). Idéal pour le travail collaboratif ou la pérennisation des pipelines de données.
